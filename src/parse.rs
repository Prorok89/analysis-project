...
// parsers for std types
    ...
    impl Parser for U32 {
		...
		fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ()> {
			...
			let nz_value = NonZeroU32::new(value).ok_or(())?;
    ...
    impl Parser for I32 {
        ...
        fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ()> {
			...
			let nz_value = NonZeroI32::new(value).ok_or(())?;
            
            Ok((&input[end_idx..], nz_value))
        }
    }
...
impl<T: Parser> Parser for StripWhitespace<T> {
    ...
    fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ()> {
        self.parser.parse(&input.trim_start()).map(
            |(remaining, parsed) : (&str, Self::Dest)| (remaining.trim_start(), parsed)
        )
    }
}
...
/// (в Rust нет чего-то, вроде variadic templates из C++)
pub fn all4<A0: Parser, A1: Parser, A2: Parser, A3: Parser>(a0: A0, a1: A1, a2: A2, a3: A3) -> All<(A0,A1,A2,A3)> {
...
/// (в Rust нет чего-то, вроде variadic templates из C++)
pub fn permutation3<A0: Parser, A1: Parser, A2: Parser>(a0: A0, a1: A1, a2: A2) -> Permutation<(A0, A1, A2)> {
	...
	fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ()> {
		...
		while !remaining.is_empty() {
			match remaining.strip_prefix(']') {
				Some(new_remaining) => return Ok((new_remaining.trim_start(), result)),
				None => {
					let (new_remaining, item) = self.parser.parse(remaining)?;
					remaining = new_remaining.trim_start()
						.strip_prefix(',').ok_or(())?.trim_start();
					result.push(item);
				},
			}
		}
...
/// (в Rust нет чего-то, вроде variadic templates из C++)
pub fn alt2<Dest,A0: Parser<Dest=Dest>,A1: Parser<Dest=Dest>>(a0: A0, a1: A1) -> Alt<(A0,A1)> {
    Alt{parser:(a0, a1)}
}
...
/// (в Rust нет чего-то, вроде variadic templates из C++)
pub fn alt3<Dest,A0: Parser<Dest=Dest>,A1: Parser<Dest=Dest>, A2: Parser<Dest=Dest>>(a0: A0, a1: A1, a2: A2) -> Alt<(A0,A1,A2)> {
    Alt{parser:(a0, a1, a2)}
}
...
/// (в Rust нет чего-то, вроде variadic templates из C++)
pub fn alt4<Dest,A0: Parser<Dest=Dest>,A1: Parser<Dest=Dest>, A2: Parser<Dest=Dest>, A3: Parser<Dest=Dest>>(a0: A0, a1: A1, a2: A2, a3: A3) -> Alt<(A0,A1,A2,A3)> {
...
/// (в Rust нет чего-то, вроде variadic templates из C++)
pub fn alt8<Dest,A0: Parser<Dest=Dest>,A1: Parser<Dest=Dest>, A2: Parser<Dest=Dest>, A3: Parser<Dest=Dest>,A4:Parser<Dest=Dest>,A5:Parser<Dest=Dest>,A6:Parser<Dest=Dest>,A7:Parser<Dest=Dest>>(a0: A0, a1: A1, a2: A2, a3: A3, a4: A4, a5: A5, a6: A6, a7: A7) -> Alt<(A0,A1,A2,A3,A4,A5,A6,A7)> {
...
use super::*;
    ...
    #[test]
    fn test_list() {
		let v: Vec<NonZeroU32> = vec![NonZeroU32::new(1).unwrap(), NonZeroU32::new(2).unwrap(), NonZeroU32::new(3).unwrap(), NonZeroU32::new(4).unwrap(),];
		let v1: Vec<NonZeroU32> = vec![NonZeroU32::new(1).unwrap(), NonZeroU32::new(2).unwrap(), NonZeroU32::new(3).unwrap(), NonZeroU32::new(4).unwrap(),];
        assert_eq!(list(stdp::U32).parse("[1,2,3,4,]".into()), Ok(("".into(),v)));
        assert_eq!(list(stdp::U32).parse(" [ 1 , 2 , 3 , 4 , ] nice".into()), Ok(("nice".into(), v1)));
